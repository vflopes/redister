...
class Logger extends EventEmitter {
	...
	setFormat (format) {
		...
		switch (format) {
		case 'pretty':
			this.on('log', (logEntry) => this._outputFunction(`${logEntry.timestamp}\t[${logEntry.address}|${logEntry.trackerId}]\t[${this._colorizeLevel(logEntry.level.toUpperCase())}]\t${logEntry.message}`));
			break;
		case 'json':
		default:
