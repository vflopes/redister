...
class Redister extends EventEmitter {
	...
	_moveOldAof (number, type) {
		const dataDirectory = `/data/${type}-${number}`;
		const newDirectory = `/data/${type}-${number}-replaced-by-${os.hostname()}`;
		try {
			fs.renameSync(dataDirectory, newDirectory);
			logger.info(`Old master directory moved (${dataDirectory}:${newDirectory})`);
		} catch (error) {
			if (error.code === 'ENOENT')
				return void 0;
			logger.error(`Error moving old master AOF directory (${dataDirectory}): ${logger.stringifyError(error)}`);
		}
	}

	async _enableAof (number, type) {
		this._moveOldAof(number, type);
		const dataDirectory = `/data/${type}-${number}/`;
	...
	async _restoreAofFile (number, type) {
		try {
			const aofDirectory = `/data/${type}-${number}-replaced-by-${os.hostname()}/appendonly.aof`;
			const stats = fs.statSync(aofDirectory);
			if (stats.size > 0) {
				logger.info(`AOF file size (${aofDirectory}): ${stats.size} bytes`);
	...
	async _setupNode () {
		...
		if (!this._discovery.peers.has(hostname))
			return this._retrySetupNode();
		...
		const dataDirectory = '/tmp/redis-node/';
		this._ensureDirectory(dataDirectory);
	...
	start () {
		...
		this._ensureDirectory('/tmp/redis-standalone')._prepare()._standaloneProcess.setConfig(
			'port',
			this._env.REDIS_STANDALONE_PORT
		).setConfig(
			'dir',
			'/tmp/redis-standalone/'
		).once(
		...
		this._discovery.on('added', (peerHostname) => {
			...
			healthcheck.on('offline', () => {
				if (healthcheck.attemptsAfterOffline >= parseInt(this._env.CUTOFF_HEALTHCHECK_ATTEMPTS)) {
					...
					this._cluster.removeNodeFromPeer(peer);
