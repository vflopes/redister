...
class RedisCluster extends EventEmitter {
	...
	async getClusterCreationTimestamp () {
		...
		if (this._standaloneNode && this._standaloneNode.isConnected) {
			try {
				const clusterCreationTimestamp = await this._standaloneNode.commands.get(
					ClusterHelpers.getTimestampKey(this._env)
				);
	...
	async _createCluster () {
		...
		await this.broadcastStandaloneCommand('set', ClusterHelpers.getTimestampKey(this._env), clusterCreationTimestamp);
		await this.broadcastStandaloneCommand('set', ClusterHelpers.getBlockedKey(this._env), 'no');
		this._creatingCluster = false;
	...
	async _ensureLeader () {
		...
		if (this.areAllNodesReady()) {
			...
			if (ClusterHelpers.isClusterEmpty(clusterInfo)) {
				this._ensuring = false;
	...
	_findNodeByClusterNodeId (clusterNodeId) {
		...
		for (const node of this._nodes.values()) {
			if (!node.cluster_nodes)
				continue;
			if (node.cluster_nodes.find((clusterNode) => ClusterHelpers.isMyself(clusterNode)
				&& clusterNode.node_id === clusterNodeId)
			)
				return node;
		}
	...
	_findFirstEpochNode () {
		...
		for (const node of this._nodes.values()) {
			...
			for (const clusterNode of node.cluster_nodes) {
				if (ClusterHelpers.isFail(clusterNode, false))
					continue;
	...
	async _releaseExpansionLock (node, lockName = 'expansion') {
		await node.redis.standalone.commands.del(`${this._env.CLUSTER_NAMESPACE}:lock:${lockName}`);
	}
	...
	async _reshard (peerNode) {
		...
		for (const clusterNode of peerNode.cluster_nodes) {
			if (
				ClusterHelpers.isMaster(clusterNode)
				&& ClusterHelpers.isFail(clusterNode, false)
				&& clusterNode.node_id !== myself.node_id
			)
				masterCount++;
		}
	...
	async _expandCluster (peerNode, masterNodeId = null) {
		...
		if (masterNodeId !== null) {
			...
			lockNode = this._findNodeByClusterNodeId(masterNodeId);
			if (lockNode === null) {
		...
		if (!lockAcquired) {
			logger.warn(`Node ${hostname} could not acquire expansion lock, aborting operation`);
			return void 0;
		}
		try {
			await RedisCli.addNode(this._nodes.get(hostname), peerNode, this._env, masterNodeId);
			await this._standaloneNode.commands.set(ClusterHelpers.getTimestampKey(this._env), Date.now());
	...
	async _forgetFailedNodes () {
		...
		for (const node of this._nodes.values()) {
			if (!node.cluster_nodes)
				continue;
			const failedNode = node.cluster_nodes.find((clusterNode) => ClusterHelpers.isFail(clusterNode, false));
			if (
				!failedNode
				|| forgotten.has(failedNode.node_id)
				|| (
					ClusterHelpers.isMaster(failedNode)
					&& node.cluster_nodes.find((clusterNode) => clusterNode.node_master === failedNode.node_id)
				)
			)
				continue;
			try {
	...
	async _failoverFailingMaster () {
		...
		try {
			const hostname = os.hostname();
			const clusterNodes = await this.getClusterNodes();
			const myself = clusterNodes.find((clusterNode) => ClusterHelpers.isMyself(clusterNode)
			&& ClusterHelpers.isSlave(clusterNode));
			if (!myself)
				return void 0;
			const master = clusterNodes.find((clusterNode) => clusterNode.node_id === myself.node_master
			&& ClusterHelpers.isFail(clusterNode, false));
			if (!master)
				return void 0;
			...
			if (masterNode)
				return void 0;
			if (ClusterHelpers.isPFail(master))
				action = 'takeover';
			lockNode = this._findFirstEpochNode();
			if (!lockNode) {
				logger.warn(`Node ${hostname} could not find his lock node, aborting operation`);
		...
		try {
			...
			await this._quorum.unsetNodeNumber('slave');
			this.emit('failover');
	...
	async _joinCluster () {
		...
		if (this._nodes.size === 0 || this._expanding || !this._localNode || !this._localNode.isConnected)
			return void 0;
		...
		const blocked = await this._standaloneNode.commands.get(ClusterHelpers.getBlockedKey(this._env));
		...
		for (const node of this._nodes.values()) {
			if (
				!node.cluster_creation_timestamp
				|| node.hostname === hostname
				|| !node.cluster_nodes
				|| (
					node.cluster_nodes
					&& node.cluster_nodes.find((clusterNode) =>
						ClusterHelpers.isFail(clusterNode, false))
				)
			)
				continue;
		...
		for (const clusterNode of peerNode.cluster_nodes) {
			if (ClusterHelpers.isFail(clusterNode, false) || !this._findNodeByClusterNodeId(clusterNode.node_id))
				continue;
			if (ClusterHelpers.isMaster(clusterNode)) {
				if (!clusterRelation.has(clusterNode.node_id))
					clusterRelation.set(clusterNode.node_id, new Set());
				masterCount++;
				continue;
			}
			if (!ClusterHelpers.isSlave(clusterNode))
				continue;
	...
	async _ensureBlockedFlag () {
		...
		if (!blocked)
			await this._standaloneNode.commands.set(ClusterHelpers.getBlockedKey(this._env), 'yes');
