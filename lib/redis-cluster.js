...
class RedisCluster extends EventEmitter {
	...
	async _reshard (peerNode) {
		...
		for (const clusterNode of peerNode.cluster_nodes) {
			if (clusterNode.node_state.includes('master') && !clusterNode.node_state.includes('fail') && clusterNode.node_id !== myself.node_id)
				masterCount++;
		}
	...
	async _forgetFailedNodes () {
		...
		for (const node of this._nodes.values()) {
			...
			if (!failedNode || forgotten.has(failedNode.node_id))
				continue;
			if (failedNode.node_state.includes('master') && node.cluster_nodes.find((clusterNode) => clusterNode.node_master === failedNode.node_id))
				continue;
			try {
	...
	async _failoverFailingMaster () {
		...
		try {
			...
			if (!myself)
				return void 0;
			const master = clusterNodes.find((clusterNode) => clusterNode.node_id === myself.node_master && clusterNode.node_state.includes('fail'));
			if (!master)
				return void 0;
			masterNode = this._findNodeByClusterNodeId(master.node_id);
			if (masterNode)
				return void 0;
			if (master.node_state.includes('fail?'))
				action = 'takeover';
			lockNode = this._findFirstEpochNode();
			if (!lockNode) {
				logger.warn(`Node ${hostname} could not find his lock node, aborting operation`);
	...
	async _joinCluster () {
		...
		for (const clusterNode of peerNode.cluster_nodes) {
			if (clusterNode.node_state.includes('fail') || !this._findNodeByClusterNodeId(clusterNode.node_id))
				continue;
