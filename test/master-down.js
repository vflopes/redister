...
const masterDown = async () => {
	...
	const [master1, master2, master3] = response.data.filter((peer) => peer.cluster_nodes.filter((clusterNode) => clusterNode.node_state.includes('myself') && clusterNode.node_state.includes('master')).length === 1);
