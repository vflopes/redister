...
const slaveDown = async () => {
	...
	const [slave1, slave2, slave3] = response.data.filter((peer) => peer.cluster_nodes.filter((clusterNode) => clusterNode.node_state.includes('myself') && clusterNode.node_state.includes('slave')).length === 1);
